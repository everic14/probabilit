...
"""
Created on Mon Jan  1 11:01:16 2024
...
def bayes_theorem(prob_a, prob_b_given_a, prob_b, decimal_places=9):
    ...
    if not (0 <= prob_a <= 1) or not (0 <= prob_b_given_a <= 1) or not (0 <= prob_b <= 1):
    ...
    if prob_b == 0 and prob_b_given_a == 1:
        raise ValueError("La probabilité de B est nulle, ce qui provoque une division par zéro.")

  

    # Correction de la formule pour éviter des résultats incohérents
    prob_a_given_b = ((prob_b_given_a * prob_a) / ((prob_b_given_a * prob_a) + ((1 - prob_b_given_a) * (1 - prob_a)), decimal_places))
